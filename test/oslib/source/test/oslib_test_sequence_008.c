 ...
 *
 * @page oslib_test_sequence_008 [8] Objects Factory
 *
...
/**
 * @page oslib_test_008_001 [8.1] Objects Registry
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.1.1] Retrieving a registered object by name, must not exist.
 * - [8.1.2] Registering an object, it must not exists, must succeed.
 * - [8.1.3] Registering an object with the same name, must fail.
 * - [8.1.4] Retrieving the registered object by name, must exist, then
 *   increasing the reference counter, finally releasing both
 *   references.
 * - [8.1.5] Releasing the first reference to the object, must not
 *   trigger an assertion.
 * - [8.1.6] Retrieving the registered object by name again, must not
 *   exist.
 * .
 */

static void oslib_test_008_001_teardown(void) {
...
/**
 * @page oslib_test_008_002 [8.2] Dynamic Buffers Factory
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.2.1] Retrieving a dynamic buffer by name, must not exist.
 * - [8.2.2] Creating a dynamic buffer it must not exists, must
 *   succeed.
 * - [8.2.3] Creating a dynamic buffer with the same name, must fail.
 * - [8.2.4] Retrieving the dynamic buffer by name, must exist, then
 *   increasing the reference counter, finally releasing both
 *   references.
 * - [8.2.5] Releasing the first reference to the dynamic buffer, must
 *   not trigger an assertion.
 * - [8.2.6] Retrieving the dynamic buffer by name again, must not
 *   exist.
 * .
 */

static void oslib_test_008_002_teardown(void) {
  dyn_buffer_t *dbp;
...
/**
 * @page oslib_test_008_003 [8.3] Dynamic Semaphores Factory
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.3.1] Retrieving a dynamic semaphore by name, must not exist.
 * - [8.3.2] Creating a dynamic semaphore it must not exists, must
 *   succeed.
 * - [8.3.3] Creating a dynamic semaphore with the same name, must
 *   fail.
 * - [8.3.4] Retrieving the dynamic semaphore by name, must exist, then
 *   increasing the reference counter, finally releasing both
 *   references.
 * - [8.3.5] Releasing the first reference to the dynamic semaphore
 *   must not trigger an assertion.
 * - [8.3.6] Retrieving the dynamic semaphore by name again, must not
 *   exist.
 * .
 */
...
/**
 * @page oslib_test_008_004 [8.4] Dynamic Mailboxes Factory
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.4.1] Retrieving a dynamic mailbox by name, must not exist.
 * - [8.4.2] Creating a dynamic mailbox it must not exists, must
 *   succeed.
 * - [8.4.3] Creating a dynamic mailbox with the same name, must fail.
 * - [8.4.4] Retrieving the dynamic mailbox by name, must exist, then
 *   increasing the reference counter, finally releasing both
 *   references.
 * - [8.4.5] Releasing the first reference to the dynamic mailbox must
 *   not trigger an assertion.
 * - [8.4.6] Retrieving the dynamic mailbox by name again, must not
 *   exist.
 * .
 */
...
static void oslib_test_008_004_execute(void) {
  ...
  /* [8.4.3] Creating a dynamic mailbox with the same name, must
     fail.*/
  test_set_step(3);
  {
    dyn_mailbox_t *dmp1;
...
/**
 * @page oslib_test_008_005 [8.5] Dynamic Objects FIFOs Factory
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.5.1] Retrieving a dynamic objects FIFO by name, must not exist.
 * - [8.5.2] Creating a dynamic objects FIFO it must not exists, must
 *   succeed.
 * - [8.5.3] Creating a dynamic objects FIFO with the same name, must
 *   fail.
 * - [8.5.4] Retrieving the dynamic objects FIFO by name, must exist,
 *   then increasing the reference counter, finally releasing both
 *   references.
 * - [8.5.5] Releasing the first reference to the dynamic objects FIFO
 *   must not trigger an assertion.
 * - [8.5.6] Retrieving the dynamic objects FIFO by name again, must
 *   not exist.
 * .
 */
...
static void oslib_test_008_005_execute(void) {
  ...
  /* [8.5.1] Retrieving a dynamic objects FIFO by name, must not
     exist.*/
  test_set_step(1);
  {
    dofp = chFactoryFindObjectsFIFO("myfifo");
...
/**
 * @page oslib_test_008_006 [8.6] Dynamic Pipes Factory
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [8.6.1] Retrieving a dynamic pipe by name, must not exist.
 * - [8.6.2] Creating a dynamic pipe it must not exists, must succeed.
 * - [8.6.3] Creating a dynamic pipe with the same name, must fail.
 * - [8.6.4] Retrieving the dynamic pipe by name, must exist, then
 *   increasing the reference counter, finally releasing both
 *   references.
 * - [8.6.5] Releasing the first reference to the dynamic pipe must not
 *   trigger an assertion.
 * - [8.6.6] Retrieving the dynamic pipe by name again, must not exist.
 * .
 */

static void oslib_test_008_006_teardown(void) {
  dyn_pipe_t *dpp;
...
static void oslib_test_008_006_execute(void) {
  dyn_pipe_t *dpp;
  ...
  /* [8.6.2] Creating a dynamic pipe it must not exists, must
     succeed.*/
  test_set_step(2);
  {
  ...
  /* [8.6.6] Retrieving the dynamic pipe by name again, must not
     exist.*/
  test_set_step(6);
  {
    dpp = chFactoryFindPipe("mypipe");
...
static const testcase_t oslib_test_008_006 = {
  "Dynamic Pipes Factory",
  NULL,
  oslib_test_008_006_teardown,
  oslib_test_008_006_execute
};
