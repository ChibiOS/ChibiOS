...
static bool fdcan_clock_stop(hal_can_driver_c *canp) {
  systime_t start, end;
...
static bool fdcan_init_mode(hal_can_driver_c *canp) {
  systime_t start, end;
...
static bool fdcan_active_mode(hal_can_driver_c *canp) {
  systime_t start, end;
 ...
 * @retval HAL_RET_HW_FAILURE  if the operation failed.
 *
 * @notapi
 */
msg_t can_lld_start(hal_can_driver_c *canp) {
  const hal_can_config_t *config = (const hal_can_config_t *)canp->config;

  if (config == NULL) {
    config = can_lld_selcfg(canp, 0U);
  }
  if (config == NULL) {
   ...
   * configuration mode */
  if (config->CCCR & FDCAN_CCCR_TEST) {
    canp->fdcan->TEST = config->TEST;
  }
...
/**
 * @brief   Resets the CAN driver cached event state.
 *
...
/**
 * @brief   Verifies a CAN configuration.
 *
 * @param[in] canp      pointer to the @p hal_can_driver_c object
...
/**
 * @brief   Selects a CAN configuration.
 *
 * @param[in] canp      pointer to the @p hal_can_driver_c object
 ...
 * @notapi
 */
void can_lld_set_filters(hal_can_driver_c *canp,
                         uint8_t num,
                         const CANFilter *cfp) {
 ...
 * @api
 */
void canSTM32SetFilters(hal_can_driver_c *canp,
                        uint8_t num,
                        const CANFilter *cfp) {
