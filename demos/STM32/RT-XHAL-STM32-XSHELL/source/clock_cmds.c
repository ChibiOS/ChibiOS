...
void cmd_clock(xshell_t *xshp, int argc, char *argv[], char *envp[]) {
  ...
  if (strcmp(argv[1], "reset") == 0) {
    ...
    chprintf(xshp->stream, XSHELL_NEWLINE_STR "Switching to default xmcuconf.h clock settings: ");
  ...
  /* Reconfiguring the SIO because clock frequencies could have changed.*/
  if (!result) {
    portab_sio_update_clock();
    if (drvSelectCfgX(&PORTAB_SIO1, 0U) == NULL) {
      result = true;
    }
  }
